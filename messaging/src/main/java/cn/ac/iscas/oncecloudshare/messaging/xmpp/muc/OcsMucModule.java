package cn.ac.iscas.oncecloudshare.messaging.xmpp.muc;
...
public class OcsMucModule extends DefaultDiscoAwareModule implements Component,
		ComponentInfoRequestListener, ItemRequestListener {
	...
	public OcsMucModule(String subdomain){
	...
	public OcsMucModule(String subdomain, Conference conference){
	...
	public void initialize(ServerRuntimeContext serverRuntimeContext){
		...
		ComponentStanzaProcessor processor=new ComponentStanzaProcessor(
				serverRuntimeContext);
...
//		processor.addHandler(new MUCIqAdminHandler(conference));
	 ...
	 */
	@Override
	protected void addItemRequestListeners(
			List<ItemRequestListener> itemRequestListeners){
		itemRequestListeners.add(this);
	}

	public List<InfoElement> getComponentInfosFor(InfoRequest request)
		...
		if(request.getTo().getNode()==null){
			...
			if(request.getTo().getResource()!=null){
				...
				Occupant occupant=room.findOccupantByNick(request.getTo()
						.getResource());
	...
	@Override
	protected void addComponentInfoRequestListeners(
			List<ComponentInfoRequestListener> componentInfoRequestListeners){
	...
	public List<Item> getItemsFor(InfoRequest request)
		...
		if(to.getNode()==null){
			// react on request send to server domain or this subdomain, but not
			// to others
	...
	private void relayDiscoStanza(Entity receiver,InfoRequest request,String ns){
		StanzaBuilder builder=StanzaBuilder.createIQStanza(request.getFrom(),
				receiver,IQStanzaType.GET,request.getID());
		...
		try{
			serverRuntimeContext.getStanzaRelay().relay(receiver,
					builder.build(),new IgnoreFailureStrategy());
		}
