...
public class GenericUtils {
	...
	public static Class<?>[] getSuperClassGenricTypes(Class<?> clazz) {
		...
		Preconditions.checkArgument(genType instanceof ParameterizedType);
		Type[] params = ((ParameterizedType) genType).getActualTypeArguments();
		Class<?>[] results = new Class<?>[params.length];
		for (int i = 0, ii = params.length; i < ii; i++){
	...
	public static Class<?> getSuperClassGenricType(Class<?> clazz, int index) {
		...
		Class<?>[] classes = getSuperClassGenricTypes(clazz);
		Preconditions.checkArgument(index < classes.length);
